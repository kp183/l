...
export default function DashboardPage() {
  ...
  // Extract unique models from traces to build model filter dropdown
  const uniqueModels = React.useMemo(() => {
    if (!tracesData?.data) return [];
    return Array.from(new Set(tracesData.data.map(t => t.name.includes("(") ? t.name.split("(")[1].replace(")", "").trim() : null).filter(Boolean)));
  }, [tracesData]);
